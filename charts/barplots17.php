...
if ($_GET["year"]!='') $today["year"]=$_GET["year"];
 ...
 if ($_GET["date"]=='') 
	{ 
	 $st=''.$today["year"].'0300000000'; 
	 $fn=''.$today["year"].'0400000000';
	 $k=1; 
	}
 else 
	{ 
...
while ($uy)
	...
	//echo $query;
     ...
     if ($ui) 
	{
	 $query = 'SELECT * FROM prdata WHERE (type=1 OR type=2) AND prm=6 AND date>'.$st.' AND device='.$ui[1].' ORDER BY date LIMIT 1';	
         $e = mysql_query ($query,$i);
	 ...
	 $query = 'SELECT * FROM prdata WHERE (type=1 OR type=2) AND prm=6 AND date<'.$fn.' AND device='.$ui[1].' ORDER BY date DESC LIMIT 1';
         $e = mysql_query ($query,$i);
         if ($e) $ui = mysql_fetch_row ($e);
         if ($ui)
	    {
...
$title='Приведенное потребление потребление горячей воды, квартиры '.$_GET["n1"].' подъезда (м3/чел.)';
