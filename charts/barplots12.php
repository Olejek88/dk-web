...
for ($tm=1; $tm<=$dy; $tm++)
    {
     ...
     $dat[$cn]=$tm.'-'.$mn;
     $query='';
     if ($_GET["n1"]==1) $query = 'SELECT SUM(value) FROM data WHERE prm=11 AND source=6 AND type=2 AND value<30 AND date='.$date1;

     if ($_GET["n1"]==2) $query = 'SELECT SUM(value) FROM data WHERE prm=11 AND source=5 AND type=2 AND value<30 AND date='.$date1;

     if ($_GET["n1"]==3) $query = 'SELECT SUM(value) FROM data WHERE prm=2 AND flat>0 AND value<200 AND type=2 AND date='.$date1;

     if ($_GET["n1"]==4) $query = 'SELECT SUM(value),AVG(value),COUNT(id) FROM prdata WHERE prm=13 AND value>10 AND type=2 AND date='.$date1;
	//echo $query;
     ...
     if ($_GET["n1"]==1) 
	{
     ...
     if ($_GET["n1"]==3) $query = 'SELECT SUM(value) FROM data WHERE prm=14 AND flat=0 AND type=2 AND date='.$date1;
