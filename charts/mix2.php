...
{
 for ($x=0;$x<$max;$x++)
 if ($date1[$x]==$uy[4]) { $datas1[$x]+=$uy[5]/4184; $cnt[$x]++; $sum[$x]+=$uy[5]/4184; }
 $uy = mysql_fetch_row ($a);
}
...
for ($i=0; $i<=$max; $i++) 
    {
     ...
     $dats[$cm]=$dat[$max-$i]; $data1[$cm]=$datas0[$max-$i]-$datas1[$max-$i]; $data0[$cm]=$datas1[$max-$i]; $data2[$cm]=$datas2[$max-$i];
     //print $dat[$max-$i].' '.$data0[$cm].' '.$data1[$cm].'<br>';
	if ($data0[$cm]>0 && $data1[$cm]>0) $cm++;
    }
...
$graph->img->SetMargin(35,35,15,23);
//----------- title --------------------
