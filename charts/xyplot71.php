...
if ($_GET["prm"]==13 && $_GET["pipe"]==2) $graph->title->Set("Потребление тепловой энергии (ГКал)");
if ($_GET["prm"]==13 && $_GET["pipe"]==0) $graph->title->Set("Тепловая энергия подающей (ГКал)");
...
if ($_GET["prm"]==12 && $_GET["pipe"]==0) $graph->title->Set("Расход подающего теплоносителя (С)");
if ($_GET["prm"]==12 && $_GET["pipe"]==1) $graph->title->Set("Расход обратного теплоносителя (С)");
if ($_GET["prm"]==4 && $_GET["pipe"]==0) $graph->title->Set("Температура подающего теплоносителя (С)");
if ($_GET["prm"]==4 && $_GET["pipe"]==1) $graph->title->Set("Температура обратного теплоносителя (С)");
