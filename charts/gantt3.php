...
$query = 'SELECT * FROM device WHERE type=1 AND ust=0 ORDER BY flat';
...
while ($ui)
	  ...
	  $dev[$cn]=$ui[1];
	  $flat[$cn]=$ui[10];
	  $query = 'SELECT COUNT(id) FROM prdata WHERE type=2 AND prm=1 AND device='.$dev[$cn];
...
for ($tn=0; $tn<$x; $tn++)
{
 $query = 'SELECT * FROM prdata WHERE type=2 AND prm=1 AND date LIKE \'%'.$dats[$tn].'%\'';
 $a = mysql_query ($query,$i);
 if ($a) $uy = mysql_fetch_row ($a);
 while ($uy)
    {   
...
$graph->title->Set("Статистика работы датчиков 2ИП - периоды передачи / выхода в эфир");
