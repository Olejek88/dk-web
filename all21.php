...
<tr><td bgcolor=#ffcf68 align=center><font class=tablz><?php $today=getdate(); print $today["year"]; ?></font></td><td bgcolor=#ffcf68 align=center colspan=6><font class=tablz>Количество тепловой энергиий (Гкал)</font></td></tr>
...
<td bgcolor=#ffcf68 align=center><font class=tablz>норм.</font></td><td bgcolor=#ffcf68 align=center colspan=2><font class=tablz>% эко</font></td>
...
$ffile="inc/all".$_GET["obj"]."_11.php";
...
{
 for ($tm=1; $tm<=22; $tm++) $data2[$tm]=$data1[$tm]=$data0[$tm]=0;
 $tm=$today["mon"];
 for ($pm=1; $pm<=22; $pm++)
    {	 
     $tod=31;
	 ...
	 if ($tm==$today["mon"]) $numm=($today["mday"]-1)*$uy[0]; else $numm=$tod*$uy[0];
	 ...
	 //echo $numm.' '.$ui[0].' '.$ui[1].' '.$ui[2].'<br>';
	    ...
	    //else if ($tm>2 && $tm<5) 
	    if ($today["mon"]==2 || $today["mon"]==1) $ui[0]=$ui[0];
	    else $ui[0]=$ui[0]+($numm-$ui[1])*($ui[2]);
 ...
 $ffile="inc/all".$_GET["obj"]."_11g.php"; $cm=1;
 $fp=fopen($ffile,"w"); $prt='charts/barplots282.php?n1=13&obj='.$_GET["obj"]; fwrite ($fp,$prt);
 for ($cn=0; $cn<22; $cn++)
 if ($data9[$cn]>0)
    {	 
...
<tr><td bgcolor=#ffcf68 align=center><font class=tablz><?php print $today["year"]; ?></font></td><td bgcolor=#ffcf68 align=center colspan=6><font class=tablz>Объем ХВС (м3)</font></td></tr>
<tr><td bgcolor=#ffcf68 align=center><font class=tablz>месяц</font></td><td bgcolor=#ffcf68 align=center><font class=tablz>инд. часть</font></td>
<td bgcolor=#ffcf68 align=center><font class=tablz>всего</font></td>
...
<?php
$cn=0; $today=getdate();
$ffile="inc/all".$_GET["obj"]."_13.php";
if (file_exists($ffile))
	{
	 $fp=fopen($ffile,"r");
	 $contents = fread ($fp, filesize ($ffile));
...
if ($data9[$cn]>1)
 ...
 $ffile="inc/all".$_GET["obj"]."_13g.php"; $cm=1;
 $fp=fopen($ffile,"w"); $prt='charts/barplots282.php?n1=8&obj='.$_GET["obj"]; fwrite ($fp,$prt);
 for ($cn=0; $cn<22; $cn++)
 if ($data9[$cn]>0)
    {	 
     $prt='&dat'.$cm.'='.$dat[$cn].'&da'.$cm.'='.$data2[$cn].'&db'.$cm.'='.$data1[$cn].'&dc'.$cm.'='.$data0[$cn]; fwrite ($fp,$prt); $cm++;
...
<tr><td bgcolor=#e8e8e8 align=center colspan=8><font class=tablz></font></td></tr>
<tr><td bgcolor=#ffcf68 align=center><font class=tablz><?php print $today["year"]; ?></font></td><td bgcolor=#ffcf68 align=center colspan=6><font class=tablz>Объем ГВС (м3)</font></td></tr>
<tr><td bgcolor=#ffcf68 align=center><font class=tablz>месяц</font></td><td bgcolor=#ffcf68 align=center><font class=tablz>инд. часть</font></td>
<td bgcolor=#ffcf68 align=center><font class=tablz>всего</font></td>
<td bgcolor=#ffcf68 align=center><font class=tablz>норматив</font></td><td bgcolor=#ffcf68 align=center colspan=2><font class=tablz>% экономии по общему объему</font></td>
<td bgcolor=#ffcf68 align=center><font class=tablz>экономия в рублях</font></td></tr>
...
<?php
$cn=0; $today=getdate();
$ffile="inc/all".$_GET["obj"]."_14.php";
if (file_exists($ffile))
	{
	 $fp=fopen($ffile,"r");
	 $contents = fread ($fp, filesize ($ffile));
...
else
{
 ...
 $ffile="inc/all".$_GET["obj"]."_14g.php"; $cm=1;
 $fp=fopen($ffile,"w"); $prt='charts/barplots282.php?n1=6&obj='.$_GET["obj"];  fwrite ($fp,$prt);
 for ($cn=0; $cn<31; $cn++)
 if ($data9[$cn]>0)
    {	 
     $prt='&dat'.$cm.'='.$dat[$cn].'&da'.$cm.'='.$data2[$cn].'&db'.$cm.'='.$data1[$cn].'&dc'.$cm.'='.$data0[$cn]; fwrite ($fp,$prt); $cm++;
...
<tr><td bgcolor=#ffcf68 align=center><font class=tablz><?php print $today["year"]; ?></font></td><td bgcolor=#ffcf68 align=center colspan=2><font class=tablz>Количество тепловой энергии (Гкал)</font></td>
...
<td bgcolor=#ffcf68 align=center><font class=tablz>норматив на факт. объем</font></td>
...
$cn=0; $today=getdate();
$ffile="inc/all".$_GET["obj"]."_15.php";
if (file_exists($ffile))
	{
	 $fp=fopen($ffile,"r");
	 $contents = fread ($fp, filesize ($ffile));
...
else
{
 ...
 for ($pm=1; $pm<=22; $pm++)
    {	 
     ...
     if ($datas4[$cn]==0) 
	{
	 $query = 'SELECT SUM(value) FROM data WHERE date>'.$sts.' AND date<'.$fns.' AND type=2 AND prm=12 AND source=5 AND flat=0';
	 $a = mysql_query ($query,$i);
	 if ($a) $uy = mysql_fetch_row ($a);
     	 if ($uy) $datas4[$cn]=$uy[0];
	}
 ...
 $ffile="inc/all".$_GET["obj"]."_15g.php"; $cm=1;
 $fp=fopen($ffile,"w"); $prt='charts/barplots282.php?n1=15&obj='.$_GET["obj"]; fwrite ($fp,$prt);
 for ($cn=0; $cn<22; $cn++)
 if ($data9[$cn]>0)
    {	 
     $prt='&dat'.$cm.'='.$dat[$cn].'&da'.$cm.'='.number_format($data2[$cn],1).'&db'.$cm.'='.number_format($data1[$cn],1).'&dc'.$cm.'='.number_format($data9[$cn],1); fwrite ($fp,$prt); $cm++;
    }
 fclose($fp);
}
$it1=$it2=$it3=$it4=$it5=$it6=0;
...
<tr><td bgcolor=#e8e8e8 align=center colspan=8><font class=tablz></font></td></tr>
<tr><td bgcolor=#ffcf68 align=center><font class=tablz><?php print $today["year"]; ?></font></td><td bgcolor=#ffcf68 align=center colspan=6><font class=tablz>Количество электроэнергии (кВт/ч)</font></td></tr>
<tr><td bgcolor=#ffcf68 align=center><font class=tablz>месяц</font></td><td bgcolor=#ffcf68 align=center><font class=tablz>инд. часть</font></td>
<td bgcolor=#ffcf68 align=center><font class=tablz>всего</font></td>
<td bgcolor=#ffcf68 align=center><font class=tablz>норматив</font></td><td bgcolor=#ffcf68 align=center colspan=2><font class=tablz>% экономии по общему объему</font></td>
<td bgcolor=#ffcf68 align=center><font class=tablz>экономия в рублях</font></td></tr>
...
<?php
$cn=0; $today=getdate();
$ffile="inc/all".$_GET["obj"]."_16.php";
if (file_exists($ffile))
	{
	 $fp=fopen($ffile,"r");
	 $contents = fread ($fp, filesize ($ffile));
...
else
{
 ...
 for ($pm=1; $pm<=22; $pm++)
    {	 
     ...
     include("time.inc");
	    ...
	    $query = 'SELECT SUM(value) FROM data WHERE date>='.$sts.' AND date<'.$fns.' AND type=2 AND flat=0 AND prm=14 AND value<2000';
	    $e = mysql_query ($query,$i); $ui = mysql_fetch_row ($e); $data1[$cn]=$ui[0];
 ...
 $ffile="inc/all".$_GET["obj"]."_16g.php"; $cm=1;
 $fp=fopen($ffile,"w"); $prt='charts/barplots282.php?n1=2&obj='.$_GET["obj"]; fwrite ($fp,$prt);
 for ($cn=0; $cn<22; $cn++)
 if ($data9[$cn]>0)
    {	 
     $prt='&dat'.$cm.'='.$dat[$cn].'&da'.$cm.'='.$data2[$cn].'&db'.$cm.'='.$data1[$cn].'&dc'.$cm.'='.$data0[$cn]; fwrite ($fp,$prt); $cm++;
...
<?php
$ffile="inc/all".$_GET["obj"]."_11g.php";
if (file_exists($ffile))
	{
	 $fp=fopen($ffile,"r");	 $contents = fread ($fp, filesize ($ffile));
	 print '<tr><td><img alt="пожалуйста подождите пока идет построение изображения" width=1100 height=400 src="'.$contents.'"></td></tr>';
	 fclose($fp);
	}
$ffile="inc/all".$_GET["obj"]."_13g.php";
if (file_exists($ffile))
	{
	 $fp=fopen($ffile,"r");	 $contents = fread ($fp, filesize ($ffile));
	 print '<tr><td><img alt="пожалуйста подождите пока идет построение изображения" width=1100 height=400 src="'.$contents.'"></td></tr>';
...
$ffile="inc/all".$_GET["obj"]."_14g.php";
if (file_exists($ffile))
	{
	 $fp=fopen($ffile,"r");	 $contents = fread ($fp, filesize ($ffile));
	 print '<tr><td><img alt="пожалуйста подождите пока идет построение изображения" width=1100 height=400 src="'.$contents.'"></td></tr>';
...
$ffile="inc/all".$_GET["obj"]."_15g.php";
if (file_exists($ffile))
	{
	 $fp=fopen($ffile,"r");	 $contents = fread ($fp, filesize ($ffile));
	 print '<tr><td><img alt="пожалуйста подождите пока идет построение изображения" width=1100 height=400 src="'.$contents.'"></td></tr>';
...
$ffile="inc/all".$_GET["obj"]."_16g.php";
if (file_exists($ffile))
	{
	 $fp=fopen($ffile,"r");	 $contents = fread ($fp, filesize ($ffile));
	 print '<tr><td><img alt="пожалуйста подождите пока идет построение изображения" width=1100 height=400 src="'.$contents.'"></td></tr>';
