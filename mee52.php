...
while ($uy)
 ...
 $query = 'SELECT SUM(value) FROM data WHERE date<'.$fn.' AND date>'.$st.' AND type=2 AND flat=0 AND prm=14';
...
//echo $query.'<br>';
 ...
 $pr00=number_format(100*($datav0-$ssum0)/$cold0,2);
...
<?php
 ...
 print '<tr><td bgcolor=#eeeeee align=center><font class=top5>Домовое потребление (+лифт и освещение)</font></td><td bgcolor=#e8e8e8><font class=tablz>'.number_format($datav0,2).' кВт</font></td>';
...
<?php
 ...
 while ($uy)
	 ...
	 print '<td align=center align=left bgcolor=#ffffff><font class=top2>'.number_format($sum0,2).'</font></td>';
	 print '<td align=center align=left bgcolor=#eeeeee><font class=top2>'.number_format(($datav0-$ssum0)*$sum0/$ssum0,2).'</font></td>';
