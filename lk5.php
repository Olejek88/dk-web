...
<?php
 ...
 $query = 'SELECT * FROM prdata WHERE type=2 AND (prm=8 OR prm=6) AND date<='.$fn.' AND date>='.$st.' AND value<1000 ORDER BY date DESC';	
 $e = mysql_query ($query,$i);
 ...
 while ($ui)
	...
	if ($device[$w]==$ui[1])
	    {
	     if ($data11[$w]==0 && $ui[2]==8)  $data11[$w]=$ui[5]; 
	     if ($data12[$w]==0 && $ui[2]==6)  $data12[$w]=$ui[5]; 
	     if ($data11[$w]>0 && $ui[2]==8)  $data01[$w]=$ui[5]; 
	     if ($data12[$w]>0 && $ui[2]==6)  $data02[$w]=$ui[5]; 
	    }
     $ui = mysql_fetch_row ($e);    
    }                                                
...
//	  $query = 'SELECT * FROM flats WHERE flat='.$uy[10];
	  $query = 'SELECT * FROM flats WHERE flat='.$dev_flat[$cm];
	  ...
	  $sum0[$cm]+=$data11[$cm]-$data01[$cm];  if ($sum0[$cm]<0) $sum0[$cm]=0; $ssum0+=$data11[$cm]-$data01[$cm];
	  $sum1[$cm]+=$data12[$cm]-$data02[$cm];  if ($sum1[$cm]<0) $sum1[$cm]=0; $ssum1+=$data12[$cm]-$data02[$cm];
