...
if ($stage4)
{
 ...
 for ($tn=1; $tn<=123; $tn++)
	{		
	 $date1[$tn]=sprintf ("%d%02d%02d000000",$ye,$mn,$tm);
	 $date2[$tn]=sprintf ("%d%02d01000000",$ye,$mn);
	 ...
	 for ($rr=0;$rr<=$flats;$rr++) {  $Qfl[$rr]=0; $Qflo[$rr]=0; $Qflg[$rr]=0; $dgvs[$rr]=0; }
	 $sumg=0;

	 $query = 'SELECT * FROM data WHERE prm=12 AND source=5 AND flat>0 AND type=4 AND date='.$date2[$tn]; echo $query.'<br>';
	 if ($a = mysql_query ($query,$i))
	 while ($uy = mysql_fetch_row ($a)) 
		{
		 $dgvs[$uy[4]]=$uy[3];
...
//echo $query;
     ...
     $QSso*=4186;
     $QSvstr*=4186;
     echo 'Qvh='.$QSvh.',QSvstr='.$QSvstr.',QSso='.$QSso.',QS='.$QS.'('.$sumq.') | S='.$ssquar.' V='.$sumq.' Vgv='.$sumg.'<br>';
     ...
     for ($rr=1;$rr<=$flats;$rr++)
     if ($QSvh-$QSso-$QSvstr>0 && $sumg>0)
    	{
	 $Qflg[$rr]=($QSvh-$QSso-$QSvstr)*$dgvs[$rr]/$sumg;
        }
     else	
        {
         $Qflg[$rr]=0;
