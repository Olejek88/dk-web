...
while ($uy)
 ...
 $query = 'SELECT * FROM prdata WHERE type=2 AND prm=2 ORDER BY date DESC LIMIT 10000';
// echo $query;
 $a = mysql_query ($query,$i);
 if ($a) $uy = mysql_fetch_row ($a);
 while ($uy)
      {          
       for ($x=0; $x<50; $x++)
       for ($w=0;$w<=$max;$w++)	
	  if ($device[$w]==$uy[1] && $uy[4]==$dat[$x]) { $data[$x][$w]=$uy[5]; break; }
       $uy = mysql_fetch_row ($a);
      }
?>
...
<tr><td width=1190px bgcolor=#ffcf68 align=center><font class=tablz3>Изменение потребления электроэнергии по дням</font></td></tr>
...
<?php
 ...
 while ($uy)
	  ...
	  print '<tr><td align=center bgcolor=#ffcf68><font class=top2>'.$flat.'</font></td>'; 
	  print '<td align=left bgcolor=#ffcf68><a href="lk_hour2.php?flat='.$flat.'&device='.$uy[1].'&n1='.$n1.'&obj='.$_GET["obj"].'"><font class=top2>'.$abn.'</font></td>';
	  print '<td align=left bgcolor=#ffcf68><font class=top2>'.$calc[$w].'</font></td>';
	  for ($x=49;$x>=0;$x--) print '<td align=center align=left bgcolor=#ffffff><font class=top2>'.number_format($data[$x][$w]+$calc[$w],0).'</font></td>';
	  print '</tr>'; $w++;
	  $uy = mysql_fetch_row ($a);
	}
?>
