<title>Система учета индивидуального потребления энергоресурсов :: Расчет теплопотребления квартир по разнице энтальпий</title>
...
<?php
 ...
 $a = mysql_query ($query,$i); $sumfl=0; $qfl=0;
 ...
 if ($_GET["date"]=='') $date='20110312';
 else $date=$_GET["date"];
?>

<tr bgcolor=#ffcf68 valign=top><td colspan=2 align=center><font class=tablz3>Расчет теплопотребления квартир по разнице энтальпий за <?php print $date; ?></font></td></tr>
<tr>
<td width=1200 valign=top>
<table width=1200 bgcolor=#ffffff valign=top cellpadding=1 cellspacing=1><tr>

<?php
for ($t=1;$t<=42;$t++)
{
 print '<td><img width=200 height=100 src="charts/barplots19.php?type='.$t.'&date='.$date.'&obj='.$_GET["obj"].'"></td>';
...
// print '<td bgcolor=#ffcf68 align=center><font class=tablz>1-9</font></td></tr>';
 ...
 while ($uy)
	 {
	  $query = 'SELECT * FROM prdata WHERE type=2 AND device='.$uy[1].' AND (prm=1 OR prm=13) AND value>0 AND date='.$date.'000000 ORDER BY date DESC LIMIT 4';
 ...
 $query = 'SELECT * FROM data WHERE type=2 AND flat=0 AND prm=12 AND source=6 AND date='.$date.'000000';
 $r = mysql_query ($query,$i);
 $ur = mysql_fetch_row ($r);
 if ($ur) $sgvs=$ur[3];

 $query = 'SELECT * FROM data WHERE type=2 AND flat=0 AND prm=13 AND source=2 AND date='.$date.'000000';
 $r = mysql_query ($query,$i);
 $ur = mysql_fetch_row ($r);
 if ($ur) $ssq=$ur[3];

 $query = 'SELECT * FROM data WHERE type=2 AND flat=0 AND prm=13 AND source=3 AND date='.$date.'000000';
 $r = mysql_query ($query,$i);
 $ur = mysql_fetch_row ($r);
 if ($ur) $ssq2=$ur[3];
 $ssq3=($ssq2-$ssq)*4168;

 $query = 'SELECT SUM(value) FROM prdata WHERE type=2 AND prm=7 AND date='.$date.'000000';
 $r = mysql_query ($query,$i);
 $ur = mysql_fetch_row ($r);
 if ($ur) $ssgvs=$ur[0];

 $query = 'SELECT * FROM prdata WHERE type=2 AND prm=7 AND date='.$date.'000000';
 $r = mysql_query ($query,$i);
 $ur = mysql_fetch_row ($r);
 while ($ur) 
	{
 ...
 while ($uy)
	 {
	  ...
	  while ($ui)
		{
		  $query = 'SELECT * FROM prdata WHERE prm=1 AND type=2 AND device='.$ui[1].' AND value>0 AND date='.$date.'000000';
		  $r = mysql_query ($query,$i);
		  $ur = mysql_fetch_row ($r);
		  $data[$cn]=$ur[5];
	  ...
	  for ($u=0; $u<=9; $u++) 
		{
		 if ($data0[$u]==0 && $ct>0) $data0[$u]=$sum/$ct;
		 $dh+=$data0[$u];
		}

	  for ($u=0; $u<=8; $u++) 
	       {		
	  	if ($u>0 && $dh>0) { $k[$str][$u]=$data0[$u]/$dh; $datas[$str][$u]=$k[$str][$u]*$irp[$str][13][0]; }
	 	else if ($data0[0]>$data0[9] && $dh>0) { $k[$str][$u]=$data0[9]/$dh; $datas[$str][$u]=$k[$str][$u]*$irp[$str][13][0]; }
		     else if ($dh>0) { $k[$str][$u]=$data0[0]/$dh; $datas[$str][$u]=$k[$str][0]*$irp[$str][13][0]; }
		//echo $k[$str][$u].' '.$datas[$str][$u].'<br>';
		$qflats[$flat[$str][$u+1]]+=$datas[$str][$u];
		if ($sumfl>0) $qoflats[$flat[$str][$u+1]]=$qod*$sflats[$flat[$str][$u+1]]/$sumfl;
	       }
 ...
 for ($u=0; $u<=8; $u++)
	{
	 ...
	 if ($ss>0) print '<td bgcolor=#eecf68 align=center><font class=tablz>'.number_format($sum/$ss,2).'</font></td>';
	 else  print '<td bgcolor=#eecf68 align=center><font class=tablz>-</font></td>';
	 print '</tr>';
	}
?>
</table></td></tr>
 ...
 for ($u=0; $u<=8; $u++)
	{
	 ...
	 for ($fl=1; $fl<=$qfl; $fl++)
	     {
	      ...
	      if ($fl==$flat[$str][$u+1])
		{
	         if ($sflats[$flat[$str][$u+1]]>0) print '<td bgcolor=#ffcf68 align=center><font class=tablz>'.$flat[$str][$u+1].'</font></td><td bgcolor=#e8e8e8 align=center><font class=tablz>'.number_format($qflats[$flat[$str][$u+1]]/$sflats[$flat[$str][$u+1]],2).'</font></td>';
		 else  print '<td bgcolor=#ffcf68 align=center><font class=tablz>'.$flat[$str][$u+1].'</font></td><td bgcolor=#e8e8e8 align=center><font class=tablz>-</font></td>';
		 $sum+=$qflats[$flat[$str][$u+1]];
		 $ss+=$sflats[$flat[$str][$u+1]];
		 $cnt++; break;
		} 
	 ...
	 print '<td bgcolor=#eecf68 align=center><font class=tablz>'.number_format($sum,2).'</font></td>';
	 print '<td bgcolor=#ffcf68 align=center><font class=tablz>'.number_format($ss,2).'</font></td>';
	 if ($ss) print '<td bgcolor=#eecf68 align=center><font class=tablz>'.number_format($sum/$ss,2).'</font></td>';
	 else  print '<td bgcolor=#eecf68 align=center><font class=tablz>-</font></td>';
	 print '</tr>';
	}
?>
	 ...
	 for ($fl=1; $fl<=$qfl; $fl++)
	     {
	      ...
	      if ($fl==$flat[$str][$u+1])
		{
	         if ($ssgvs>0) print '<td bgcolor=#ffcf68 align=center><font class=tablz>'.$flat[$str][$u+1].'</font></td><td bgcolor=#e8e8e8 align=center><font class=tablz>'.number_format($ssq3*($qgvs[$flat[$str][$u+1]]/$ssgvs),2).'</font></td>';
		 else print '<td bgcolor=#ffcf68 align=center><font class=tablz>'.$flat[$str][$u+1].'</font></td><td bgcolor=#e8e8e8 align=center><font class=tablz>н/а</font></td>';
		 if ($ssgvs>0) $sum+=$ssq3*($qgvs[$flat[$str][$u+1]]/$ssgvs);
		 $ss+=$sflats[$flat[$str][$u+1]];
		 $cnt++; break;
		} 
	     } 
	 ...
	 for ($fl=1; $fl<=$qfl; $fl++)
	     {
	      ...
	      if ($fl==$flat[$str][$u+1])
		{
	         if ($ssgvs>0) print '<td bgcolor=#ffcf68 align=center><font class=tablz>'.$flat[$str][$u+1].'</font></td><td bgcolor=#e8e8e8 align=center><font class=tablz>'.number_format($ssq3*($qgvs[$flat[$str][$u+1]]/$ssgvs)+$qflats[$flat[$str][$u+1]]+$qoflats[$flat[$str][$u+1]],2).'</font></td>';
		 else print '<td bgcolor=#ffcf68 align=center><font class=tablz>'.$flat[$str][$u+1].'</font></td><td bgcolor=#e8e8e8 align=center><font class=tablz>н/а</font></td>';
		 if ($ssgvs>0) $sum+=$qoflats[$flat[$str][$u+1]]+$qflats[$flat[$str][$u+1]]+$ssq3*($qgvs[$flat[$str][$u+1]]/$ssgvs);
