...
<?php
 print '<img width=800 height=400 src="charts/barplots.php?type=1&obj='.$_GET["obj"].'">';
 print '<img width=800 height=400 src="charts/barplots.php?type=2&obj='.$_GET["obj"].'">';
 print '<img width=800 height=400 src="charts/barplots.php?type=3&obj='.$_GET["obj"].'">';
?>
</td>
...
<?php
 ...
 while ($uy)
	  ...
	  print '<font class=tablz>'.$uy[3].'</font></a>';
	  print '</td>';
	  $query = 'SELECT * FROM prdata WHERE type=2 AND device='.$uy[1].' ORDER BY date DESC,prm,pipe';
	  $e = mysql_query ($query,$i);
	  ...
	  while ($ui)
		  ...
		  if (!$ui[5])
			{
			 $query = 'SELECT * FROM hours WHERE type=1 AND device='.$uy[1].' AND prm='.$ui[2].' AND pipe='.$ui[7].' AND value>0 ORDER BY date DESC';
